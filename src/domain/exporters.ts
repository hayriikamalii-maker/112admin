...
function requestAssignedOnShift(day: ScheduleDay | undefined, staffId: string, shift?: DriverShift) {
  ...
  return [day.chiefId, day.yspId, day.dayDriverId, day.nightDriverId, day.fullDriverId].includes(staffId);
}
...
function tableRows(_station: Station, schedule: Schedule, staff: Staff[], holidays: PublicHoliday[]) {
  return schedule.days
    .map((day) => {
      ...
      cells.push(`<td>${escapeHtml(staffName(staff, day.chiefId))}</td>`);
      cells.push(`<td>${escapeHtml(staffName(staff, day.yspId))}</td>`);
...
function scheduleMetrics(schedule: Schedule, staff: Staff) {
  return schedule.days.reduce(
    (total, day) => {
      if (day.chiefId === staff.id || day.yspId === staff.id) {
